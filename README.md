...
- **System Automation**: `browser-use` (Playwright), `sherlock-project` (OSINT).
- **Document & Media Compile**: `@marp-team/marp-cli` (Slides), `Cytoscape.js` (Codebase network map).
...
- **Persona Modeling**: Automated draft generation constrained to a specific Gen-Z Indonesian tone (casual chat contractions, no hyphens, strict emoji limits, under 500 characters).
...
- **Interaction Scan**: Runs every 30 minutes, filtering out toxic/spam comments (e.g. slot, promo) and auto-replying to simple greetings.
